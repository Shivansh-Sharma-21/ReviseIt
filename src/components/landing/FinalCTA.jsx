...
const FinalCTA = ({ onGetStarted }) => {
    ...
    // Animate words upwards as we scroll down into this section
    const yTransform = useTransform(scrollYProgress, [1, 0.8], isMobile ? [0, 0] : [80, 0]);
    ...
    return (
        <section className="h-screen relative overflow-hidden flex items-center justify-center border-t border-slate-200 dark:border-white/5 z-30">
            ...
            <div className="absolute inset-0 -z-10 overflow-hidden pointer-events-none">
                {/* Linear Moving Grid - Native CSS Animation with Hardware Acceleration */}
                <div 
                    className={`absolute -top-10 -left-10 w-[calc(100%+80px)] h-[calc(100%+80px)] opacity-40 dark:opacity-20 transition-all duration-1000 ${
                        isMobile ? 'moving-grid-bg-mobile' : 'moving-grid-bg'
                    }`}
                />
                ...
                <div className="flex flex-col items-center group/cta">
                    <h3 className="text-5xl sm:text-7xl md:text-8xl font-black text-slate-900 dark:text-white tracking-tighter mb-10 uppercase leading-[1.1] flex flex-wrap justify-center overflow-hidden py-4">
                        {titleWords.map((word, wordIdx) => {
                            const isGradient = wordIdx > 0; // "The" and "Gap"
                            return (
                                <motion.span 
                                    key={wordIdx} 
                                    style={isMobile ? {} : { y: yTransform }}
                                    className={`inline-block mx-2 will-change-transform ${
                                        isGradient 
                                    ...
                                    transition={{ duration: 0.6, delay: isMobile ? 0 : wordIdx * 0.15, type: "spring", stiffness: 90 }}
                                >
                                    {word}
                                </motion.span>
                            );
                    ...
                    <motion.p 
                        ...
                        transition={{ duration: isMobile ? 0.05 : 0.8, delay: isMobile ? 0 : 0.3 }}
                    ...
                    <div className="relative z-50">
                        <motion.button
                            ...
                            viewport={{ once: true, margin: isMobile ? "200px" : "0px" }}
                            transition={{ type: "spring", bounce: 0.4, delay: isMobile ? 0 : 0.4 }}
